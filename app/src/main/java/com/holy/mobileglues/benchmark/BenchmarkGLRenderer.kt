/*
 * Holy MobileGlues — BenchmarkGLRenderer
 * Kaynak: Star1xr/ZalithLauncher2Plus BenchmarkGLRenderer.kt (149 satır, GLES20 + GLSurfaceView)
 * Uyarlama: Holy renderer karşılaştırması için; Türkçe notlar eklendi, hatalar düzeltildi.
 *
 * Ne yapar: 500 dörtgen × 15 sn ham draw-call stresi. Her karede 500× glDrawArrays,
 * frame sürelerinden avg/min/p99/stability/score hesaplar. Tek GL program + tek VBO,
 * her çizimde sadece uniform değişir — driver'ın draw submission maliyeti ölçülür, shader değil.
 */
...
/**
 * Tek bir koşunun özeti. Puan = avgFps × kararlılık — jitter cezalandırılır.
 * @param avgFps  Ortalama FPS (1000 / ortalama frame süresi)
 * @param minFps  En düşük FPS (1000 / en uzun frame)
 * @param p99Fps  %99'luk dilim FPS — tekil takılmalar ayıklanır
 * @param stabilityPct  min/avg × 100, 0..100 (düşük = DVFS/thermal dalgalanma)
 * @param score  avgFps × stability — sıralama anahtarı
 * @param frameCount  Ölçülen kare sayısı (20'den azsa sonuç geçersiz sayılır)
 */
...
/**
 * GLES20 + GLSurfaceView renderer. 15 sn boyunca her kare 500 çizim.
 *
 * @param durationMs  Koşu süresi, varsayılan 15_000 ms (Plus ile aynı; düşük cihaz için 8000 önerilir)
 * @param onProgress  Kalan saniye (0..15) — overlay geri sayım için
 * @param onComplete  Tamamlandığında tek çağrı; frameTimes.size > 20 garantili
 */
class BenchmarkGLRenderer(
    ...
    // Shader'lar Plus ile aynı; sadece yorum eklendi.
    // Vertex: aPos'u döndürüp 0.035 küçültüp grid offset uygular — draw büyüklüğü sabit,
    // driver'ın vertex fetch maliyeti değil, draw-call sayısı ölçülür.
    private val VERT = """
        attribute vec2 aPos;
        uniform float uRot;
        uniform vec2 uOff;
    ...
    // Fragment: rotasyonla renk kayar — opt. ölü kod elemesini engeller.
    private val FRAG = """
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // 500 çizim: 25 sütun × 20 satır grid, her hücre ±0.082/0.1 offset, rotasyon 0.72° adımlı.
        // Plus ile birebir aynı yük — sonuçlar Plus ile kıyaslanabilir.
        repeat(500) { i ->
    ...
    private fun compileProgram(): Int {
        fun shader(type: Int, src: String): Int {
            ...
            // Plus'ta hata kontrolü yok; Holy'de de aynı bırakıldı — logcat'te aranır.
            return id
        }
